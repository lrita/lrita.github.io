...
当访问单元的状态时，需要拉升WL的电平。使得$$ \begin{align*} {BL} \end{align*} $$和$$\overline{BL}$$上可以读取状态。如果需要覆盖单元状态，先将$${BL}$$和$$\overline{BL}$$设置为期望的值，然后升起WL电平。由于外部的驱动强于内部的4个晶体管（M<sub>1</sub> - M<sub>4</sub>），所以旧状态会被覆盖。
